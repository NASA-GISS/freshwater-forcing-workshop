...
ds = ds.rio.reproject('EPSG:3413')

xx,yy = np.meshgrid(ds['x'],ds['y'])
ds['xx'] = (('y','x'), xx); ds['yy'] = (('y','x'), yy)
ds = ds.where(ds['melt_GL'].fillna(0) != 0)
xxrange = [ds['xx'].min().values, ds['xx'].max().values]
yyrange = [ds['yy'].min().values, ds['yy'].max().values]

ds = ds.sel({'x':slice(xxrange[0], xxrange[1]),
             'y':slice(yyrange[1], yyrange[0])}, drop=True)

g = np.log10(ds['melt']).plot(x='x', y='y',
...
Problem: Many isolate tracks

+ Smoothing by a lot would remove the high melt cells near the coast...
+ Cutting off cells with bottom X % of melt doesn't create a continuous blob of melt either...

Algorithm:
+ Make outline of largest clump
+ Smooth outline (sort-of convex hull but not quite)
+ Crop by coastlines (Greenland, Canada, Iceland, Svalbard, etc.)
+ Set new area equal to old melt values where both exist
+ Set new area equal to some low amount (median) where new area but no old melt
+ Rescale so total melt is unchanged

#+BEGIN_SRC bash :exports both :results verbatim
export GRASS_OVERWRITE=1

# v.import input=${DATADIR}/Morlighem_2017/GL_ice.gpkg output=ice
# v.to.rast input=ice output=ice use=val val=1
# v.import input=${DATADIR}/Morlighem_2017/GL_land.gpkg output=land
# v.to.rast input=land output=land use=val val=1
# r.mapcalc "land = if(isnull(ice) & isnull(land), null(), 1)"

roi=NW # debug
for roi in NO NE SE SW CW NW CE; do
  eval $(r.univar -g -e ${roi})
  roisum=${sum}
  roimedian=${median}

  # Shrink area a bit by removing all cells with bottom X % of melt
  r.null map=${roi} setnull=0
  eval $(r.univar -eg map=${roi} percentile=10 | grep percentile) # percentile_10
  r.mapcalc "r_cutoff = if(${roi} > ${percentile_10}, 1, null())"

  # shrink by 1 and grow by 1 to again remove individual lines
  r.grow input=r_cutoff output=r_shrink radius=-1.01
  r.grow input=r_shrink output=r_grow radius=1.01
  
  # Limit to largest clump (mostly removes single iceberg trails)
  r.clump input=r_grow output=clumps --o
  main_clump=$(r.stats -c -n clumps sort=desc | head -n1 | cut -d" " -f1)
  r.mapcalc "r_main = if(clumps == ${main_clump}, 1, null())"

  # buffer by X m to get a smooth border
  r.grow.distance -m input=r_main distance=distance
  r.mapcalc "r_buffer = distance < 100000" # 100 km

  # Expand original melt map with average of neighbors
  r.neighbors input=${roi} output=r_neighbor method=average size=7 weighting_function=gaussian weighting_factor=1

  # Convert new area to melt w/ median values where filled in
  r.mask -i land
  r.mapcalc "r_newmelt = if(r_buffer, if(not(isnull(${roi})), ${roi}, r_neighbor))"
  r.mask -r

  # get rid of islands
  r.mapcalc "r_clumps = if(r_newmelt)"
  r.reclass.area -c input=r_clumps output=r_areas value=1000000 mode=lesser method=rmarea
  r.mapcalc "r_noisland = if(r_areas == 1, r_newmelt, null())"
  
  # rescale to total melt from original map
  eval $(r.univar -g r_newmelt) # get $sum
  r.mapcalc "${roi}_notail = (r_noisland / ${sum}) * ${roisum}"
  r.colors -g map=${roi}_notail,${roi} color=viridis
done

# x=SE; g.gui.mapswipe first=$x second=${x}_notail # mode=mirror
...
# convert to m-2
for roi in NO NE SE SW CW NW CE GL; do
  eval $(r.univar -g ${roi}_notail | grep sum)
  r.mapcalc "${roi}_m2 = ${roi}_notail / $sum / area"
...
r.mapcalc "y = y()"
r.mapcalc "area = area()"
r.out.gdal -c -m input=area output=./tmp/area.tif format=GTiff type=Float64 createopt="COMPRESS=DEFLATE" # used in AQ processing
...
for roi in NO NE SE SW CW NW CE GL; do
  echo "Processing ROI: ${roi}"
  r.proj project=G_3413 input=${roi}_m2 output=${roi}
  # rescale
  eval $(r.univar -g ${roi} | grep sum)
  r.mapcalc "${roi} = ${roi} / $sum / area"
done
#+END_SRC
